...
factors_analysis <- data %>%
  ...
  summarise(
    ...
    Inter_caste = mean(Marital_Satisfaction == "High" & `Inter-Caste` == "Yes") * 100,
    Même_caste = mean(Marital_Satisfaction == "High" & `Inter-Caste` == "No") * 100
  ) %>%
...
data_numeric <- data %>%
  mutate(
    ...
    Inter_caste_bin = ifelse(`Inter-Caste` == "Yes", 1, 0),
    Inter_religion_bin = ifelse(`Inter-Religion` == "Yes", 1, 0)
  )
